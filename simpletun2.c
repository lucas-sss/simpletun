...
int main(int argc, char *argv[])
{
    ...
    unsigned char VPN_LABEL[VPN_LABEL_LEN] = {0x10, 0x10};
    ...
    // 把虚拟网卡fd加进epoll
    // event_data_t *edt = (event_data_t *)malloc(sizeof(event_data_t));
    // if (edt == NULL)
    // {
    //     perror("malloc event_data_t");
    // }
    // memset(edt, 0, sizeof(event_data_t));
    // edt->fd = tunfd;
    // edt->tun = 1;
    // memset(&epev, 0, sizeof(epev));
    ...
    // epev.data.fd = tunfd;
    // epev.data.ptr = edt;
    // epoll_ctl(eFd, EPOLL_CTL_ADD, tunfd, &epev);
